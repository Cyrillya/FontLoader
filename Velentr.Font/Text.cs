...
namespace Velentr.Font
{
    ...
    public abstract class Text
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 position, Color color)
        {
            ...
            {
                spriteBatch.Draw(Characters[i].Character.GlyphCache.Texture, Characters[i].Position + position, Characters[i].Character.Boundary, color, 0f, Vector2.Zero, Vector2.One, SpriteEffects.None, 0f);
            }
        }
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 position, Color color, float rotation, Vector2 origin, Vector2 scale, SpriteEffects effects, float layerDepth)
        {
            ...
            if (_calculationCache.Exists(key))
            {
                ...
                for (var i = 0; i < Characters.Count; i++) {
                    spriteBatch.Draw(Characters[i].Character.GlyphCache.Texture, positions[i], Characters[i].Character.Boundary, color, rotation, origin, scale, effects, layerDepth);
                }
            }
            else
            {
                ...
                // ReSharper disable once ForCanBeConvertedToForeach
                for (var i = 0; i < Characters.Count; i++) {
                    var textCharacter = Characters[i];
                    var characterPosition = textCharacter.Position;
                    Vector2.Transform(ref characterPosition, ref transformation, out characterPosition);
                    positions.Add(characterPosition);
                    spriteBatch.Draw(textCharacter.Character.GlyphCache.Texture, characterPosition, textCharacter.Character.Boundary, color, rotation, origin, scale, effects, layerDepth);
                }
